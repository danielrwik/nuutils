function gblurt,inputim,sig,extent
...
xdim=n_elements(inputim[*,0])+2*gdim
ydim=n_elements(inputim[0,*])+2*gdim
newim=fltarr(xdim,ydim)
for i=gdim,xdim-1-gdim do for j=gdim,ydim-1-gdim do $
      newim[i-gdim:i+gdim,j-gdim:j+gdim]+=inputim[i-gdim,j-gdim]*gim
...
newim=newim[gdim:xdim-gdim-1,gdim:ydim-gdim-1]
