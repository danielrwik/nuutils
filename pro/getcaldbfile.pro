function getcaldbfile,type,ab,par1,par2,par3,par4,par5,refmjd=refmjd
...
if type eq 'psf' then begin
    ...
    ii=where(str(indx.cal_cnam) eq cnam and indx.cal_qual eq 0 and $
                str(indx.instrume) eq inst)
    if keyword_set(refmjd) and n_elements(ii) gt 1 then begin
        tdiff=refmjd-indx[ii].ref_time
        jj=where(tdiff ge 0.)
        ii=ii[jj]
        tdiff=refmjd-indx[ii].ref_time
        blah=min(tdiff,kk)
...
if n_elements(returnstr) gt 1 then begin
    print,'WARNING: GETCALDBFILE found multiple valid files for '+type
    print,'          should use keyword refmjd to also filter on date'
endif
